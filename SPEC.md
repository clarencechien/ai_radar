...
### v0.4.4(量測 + 模擬帳本,2026-09-11)
- **量測先於調參**:`measure.py` 把 tracer 樣本算成 PASS/EXCLUDE vs **全宇宙基準**、n_eff(不重疊視窗 × 群數,避免拿 n=3000 自欺)、卡報酬依透鏡、槓桿卡 **substitute gap**(實際 − delta 應得)、凸性卡**事件覆蓋率**、籃子 IV 走勢。基準數字進 `MEASURE.md`。
- **追蹤規則修正(非閾值)**:凸性卡到期在催化劑後 0–55 天,統一「到期前 21 天停追」= 事件永遠在盲區(45 張只 4 張跨過)→ 停追天數依透鏡各設(槓桿 21、凸性 0)。標記加 bid/ask/IV;卡加進場 IV/vega/theta;每晚記 bench(SMH/SPY)。
- **模擬帳本由 state 推導**(`paper.py`),不另存狀態:進場/出場規則明文、ask→bid 成本、delta/vega/theta 貢獻拆解、SMH 同期對照、CENSORED 不計分。
- **regime 觀察欄位**(`regime.py`):IV 水位/20 日變化/自身 percentile 平均、籃子 20/60 日報酬與漲家數,每晚記錄、印在報告頂端,**不當閘**;短窗價格動能在 07–09 樣本裡是均值回歸,順風燈得等兩季條件式報酬再定。
- **§8 原則不變**:閾值凍結;帳本先量對,再談 edge。
