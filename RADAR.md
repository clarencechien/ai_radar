...
> 2026-07-03T15:49+00:00 · 這是**排除法濾網**:只負責剔掉「不能買/不要買」,留下的交給人判斷。**不是投資建議;要不要出手、出多少,永遠由人決定。**

## 今晚結論

掃描 **18 檔** AI 相關股 → 通過濾網 **3 檔**、排除 **15 檔**、資料缺失 **0 檔**。(宇宙來源:ETF 持股(AIQ:top10、DRAM:top10、SMH:top10、SOXX:top10;18 檔))

以下每檔附「若要動手,規則會挑哪一張合約」——**是條件式答案,不是買進訊號**:

### AAPL(NO_DATA桶)· 事件凸性
- 規則挑的合約:**2026-08-21 到期、履約價 $345 的買權**,一張約 **$205**(報價 $2.05×100)。
- 白話:財報/事件 T-27 天的小額大賠率——股價需漲過 **+12.4%** 才開始賺;若事件把股價推到 **+30%**,光內含價值就是本金的 **27.4 倍**。輸的上限就是那一張的錢。

### AVGO(加速器桶)· 長期槓桿
- 規則挑的合約:**2028-12-15 到期、履約價 $350 的買權**,一張約 **$12,975**(報價 $129.75×100)。
- 白話:用約 1.98 倍槓桿參與 AVGO 的長期走勢(深價內、時間站在你這邊較久);股價漲過 **+33.1%** 開始賺。無近期事件時鐘。

### MU(記憶體桶)· 長期槓桿
- 規則挑的合約:**2028-12-15 到期、履約價 $1100 的買權**,一張約 **$49,600**(報價 $496.0×100)。
- 白話:用約 1.48 倍槓桿參與 MU 的長期走勢(深價內、時間站在你這邊較久);股價漲過 **+63.6%** 開始賺。無近期事件時鐘。

## 排除了誰(一句話)

被剔掉的 15 檔:14 檔「沒有夠便宜的樂透型合約」、1 檔「找不到條件合適的長期買權」。明細見下方 Details。

---

## Details(開發者)

- 資料時戳 2026-07-03T15:49+00:00 · 無風險利率 短 3.663% / 長 4.130%(缺→4.50%)
...
|---|---|---|---|---|---|---|---|---|---|
| AAPL | NO_DATA | convexity | T-27 | 2026-08-21 $345C | $2.05 | 0.142 | 21.4x | +12.4% | 若+30% 內含 27.4× |
| AVGO | 加速器 | leverage | — | 2028-12-15 $350C | $129.75 | 0.714 | 1.98x | +33.1% | — |
| MU | 記憶體 | leverage | — | 2028-12-15 $1100C | $496.0 | 0.752 | 1.48x | +63.6% | — |
...
- AAPL:2/60 筆(未滿 → edge 閘用 ratio 後備)
- AMAT:2/60 筆(未滿 → edge 閘用 ratio 後備)
- AMD:2/60 筆(未滿 → edge 閘用 ratio 後備)
- AMZN:1/60 筆(未滿 → edge 閘用 ratio 後備)
- ASML:1/60 筆(未滿 → edge 閘用 ratio 後備)
- AVGO:2/60 筆(未滿 → edge 閘用 ratio 後備)
- CEG:1/60 筆(未滿 → edge 閘用 ratio 後備)
- CSCO:2/60 筆(未滿 → edge 閘用 ratio 後備)
- GEV:1/60 筆(未滿 → edge 閘用 ratio 後備)
- GOOGL:1/60 筆(未滿 → edge 閘用 ratio 後備)
- INTC:2/60 筆(未滿 → edge 閘用 ratio 後備)
- KLAC:2/60 筆(未滿 → edge 閘用 ratio 後備)
- LRCX:2/60 筆(未滿 → edge 閘用 ratio 後備)
- META:1/60 筆(未滿 → edge 閘用 ratio 後備)
- MRVL:2/60 筆(未滿 → edge 閘用 ratio 後備)
- MSFT:1/60 筆(未滿 → edge 閘用 ratio 後備)
- MU:2/60 筆(未滿 → edge 閘用 ratio 後備)
- NVDA:2/60 筆(未滿 → edge 閘用 ratio 後備)
- NXPI:2/60 筆(未滿 → edge 閘用 ratio 後備)
- ORCL:2/60 筆(未滿 → edge 閘用 ratio 後備)
- PLTR:1/60 筆(未滿 → edge 閘用 ratio 後備)
- QCOM:2/60 筆(未滿 → edge 閘用 ratio 後備)
- SNDK:2/60 筆(未滿 → edge 閘用 ratio 後備)
- STX:2/60 筆(未滿 → edge 閘用 ratio 後備)
- TSM:2/60 筆(未滿 → edge 閘用 ratio 後備)
- TXN:2/60 筆(未滿 → edge 閘用 ratio 後備)
- VST:1/60 筆(未滿 → edge 閘用 ratio 後備)

### shadow tracer(collect_only)

- 掃描累計 62 筆、outcome 回填 0 筆
