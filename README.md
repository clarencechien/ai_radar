...
- [x] **Block 6 — 量測 + 模擬帳本(2026-09-11)** — `measure.py`(PASS/EXCLUDE vs 全宇宙、n_eff、卡報酬、substitute gap、凸性事件覆蓋、IV 走勢)+ `paper.py`(由 state 推導的模擬帳本:進場/出場規則、ask→bid 成本、delta/vega/theta 拆解、SMH 對照)+ `regime.py`(IV 水位/籃子報酬觀察欄位,**只記錄不裁決**)。追蹤規則修正:凸性卡追到到期日(原 21 天停追 = 事件在盲區)、標記加 bid/ask/IV、每晚記 bench。基準數字與一個月後對照表見 [`MEASURE.md`](./MEASURE.md)。閾值全部凍結未動。
...
nightly Action 只認 **main** 上的程式;把這個分支合進 main 之後,每晚才會開始記 bid/ask/IV 標記、bench、regime,凸性卡才會追到到期日。合併後第一個月不要改任何閾值,月中跑 `python notebooks/measure.py` 對照 [`MEASURE.md`](./MEASURE.md) §3。
...
## 量測(一個月後跑)
```bash
python notebooks/measure.py     # 離線、只讀 state/;對照 MEASURE.md §3
```
